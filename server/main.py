...
def create_session(username: str) -> str:
	sid = uuid.uuid4().hex
	conn = get_conn()
	conn.execute("INSERT INTO sessions(session_id, username) VALUES (?, ?)", (sid, username))
	conn.commit()
	conn.close()
	return sid
...
async def accept_friend_request(request_id: str, username: str = Depends(get_current_username)):
	...
	cur.execute("INSERT OR IGNORE INTO friends(user, friend) VALUES (?, ?)", (username, from_user))
...
if __name__ == "__main__":
	...
	parser.add_argument("--data", default=None, help="Path to sqlite data file to use (overrides default)")
